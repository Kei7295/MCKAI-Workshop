...
fun registerNetworkTools(r: ToolRegistry) {
    ...
    r.register(ToolMetadata(
        ...
        try {
            ...
            val text = response.body?.string() ?: "(空响应)"
            val limited = if (text.length > 15000) text.take(15000) + "\n...(已截断)" else text
            limited
        } catch (e: Exception) {
            "获取失败：${e.message}"
        }
    ...
    r.register(ToolMetadata(
        ...
        parameters = buildJsonObject {
            ...
            put("properties", buildJsonObject {
                ...
                put("num_results", buildJsonObject { put("type", JsonPrimitive("integer")); put("description", JsonPrimitive("返回结果数量，默认5")) })
            })
